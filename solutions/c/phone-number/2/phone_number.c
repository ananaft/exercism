...
char *phone_number_clean(const char *input) {
	...
	uint8_t validation = validate(cleaned_input);
	if (validation == VALID_10) {
		memcpy(output, cleaned_input, OUTPUT_LEN);
		output[OUTPUT_LEN] = '\0';
	} else if (validation == VALID_11) {
		memcpy(output, &cleaned_input[1], OUTPUT_LEN);
		output[OUTPUT_LEN] = '\0';
	} else {
